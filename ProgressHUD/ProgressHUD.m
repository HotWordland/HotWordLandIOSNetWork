...
@synthesize interaction, window, background, hud, spinner, image, label, smallActivityIndicatorView;
...
//	if (spin) [spinner startAnimating]; else [spinner stopAnimating];
    	if (smallActivityIndicatorView) [smallActivityIndicatorView startActivity]; else [smallActivityIndicatorView stopActivity];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
    /*
     2014 - 10 -29 修改样式为wp8加载风格 - by情热大陆
     */
	...
	if (spinner == nil)
	{
        ...
        smallActivityIndicatorView = [[DTIActivityIndicatorView alloc] initWithFrame:CGRectMake(19.0, 5.0, 58.0, 58.0)];
//        [smallActivityIndicatorView setCenter:spinner.center];
        smallActivityIndicatorView.indicatorColor = HUD_SPINNER_COLOR;
        smallActivityIndicatorView.indicatorStyle = @"wp8";
        
//        [smallActivityIndicatorView startActivity];
        
	}
	if (spinner.superview == nil)
    {
//        [hud addSubview:spinner];
       [hud addSubview:smallActivityIndicatorView];
    }
	//---------------------------------------------------------------------------------------------------------------------------------------------
	if (image == nil)
	{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    [smallActivityIndicatorView setCenter:CGPointMake(label.center.x, label.center.y-42)];
}
	...
	if (self.alpha == 0)
	{
		...
		[UIView animateWithDuration:0.15 delay:0 options:options animations:^{
            ...
            if (self->image.image!=nil) {
                [self->smallActivityIndicatorView setHidden:YES];
            }
            else
            {
                [self->smallActivityIndicatorView setHidden:NO];
            }
		} completion:nil];
	}else [self->smallActivityIndicatorView setHidden:YES];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	@autoreleasepool
	{
		...
		NSTimeInterval sleep = length * 0.15 + 0.5;
